...
def start_ollama():
    ...
    if is_port_in_use(11434):
        log("Ollama is already running (Port 11434 active).", "✅", GREEN)
        return None
...
def start_mlflow():
    ...
    if is_port_in_use(5001):
        log("MLflow is likely already running (Port 5001 active).", "✅", GREEN)
        return None
    ...
    try:
        ...
        for _ in range(10):
            if is_port_in_use(5001):
                log("MLflow UI started at http://127.0.0.1:5001", "✅", GREEN)
                webbrowser.open("http://127.0.0.1:5001")
                return proc
            time.sleep(1)

        log("MLflow started (background).", "✅", GREEN)
        webbrowser.open("http://127.0.0.1:5001")
        return proc
